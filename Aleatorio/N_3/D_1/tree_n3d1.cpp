...
void tree_n3d1(float inr_1[2], float ini_1[2],float inr_2[2], float ini_2[2],float inr_3[2], float ini_3[2], float outr[8], float outi[8], float mr_1[4][4], float mi_1[4][4])
{
...
#pragma HLS INTERFACE mode=s_axilite port=mi_1
  ...
  for(int i = 0; i<4; i++) {
	  for(int j = 0; j<2; j++){
	  outr[j+2*i]=(mrxr[i]-mixi[i])*inr_3[j]-(mixr[i]+mrxi[i])*ini_3[j];
	  outi[j+2*i]=(mrxr[i]-mixi[i])*ini_3[j]+(mixr[i]+mrxi[i])*inr_3[j];
	  }
  }
